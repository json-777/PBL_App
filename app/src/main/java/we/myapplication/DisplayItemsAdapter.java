...
public class DisplayItemsAdapter extends BaseAdapter {
    ...
    public void addItemsFromISNB(final String ISNB){
        acquireBookFromeInternet(UrlInformation.getBookUrlInfo(ISNB),ISNB);
    }
     ...
     * @param ISBN　ISNB番号
     */
    private  void acquireBookFromeInternet(String url,final String ISBN){
        VolleyXmlRequest mRequest  = new VolleyXmlRequest(
            ...
            public synchronized void onResponse(InputStream response) {
                Book book = null;
                try {
                    book = XmlParser.domParse(response);
                ...
                if(!book.getTitle().equals("")){
                    book.setISBN(ISBN);
                    book.setImage_URL(UrlInformation.getImageURL(ISBN,0));
